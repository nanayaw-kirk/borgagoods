...
<template>
  <div>
    ...
    <div class="container grid gap-4 md:gap-8 lg:gap-12 pt-2 md:pt-4">


     <div class="px-2 md:px-0" v-if="homePage?.categories?.length > 0">
      <home-categories class="contain-slider" :list="homePage?.categories"/>
    </div>
  ...
  <!--   <nuxt-link :to="{name: 'black-friday'}" class="w-full aspect-video md:h-[450px] bg-black relative">
    ...
    <div class="px-2 md:px-0" v-if="homePage?.products?.length > 0">
      <home-offers :data="homePage?.products"/>
    </div>
...
async function loadData(){
  loading.value = true
  const {data} = await useApiFetch("/pages/home?bypass_cache")
  if(data?.value){

<template>
  <div>
    ...
    <Head>
      <Title>Discover the latest merchandise drops on {{$appName}}</Title>
    ...
    <div class="container p-2 md:py-4">
      <div class="flex flex-col w-full px-1 sm:px-0">
        <h2 class="text-center sm:text-left text-xl md:text-2xl font-bold">
         Fresh Drops. New Vibes.
       </h2>
       <p class="text-center sm:text-left text-foreground/70 text-sm md:text-base">
        Stay ahead of the game with the latest merch drops.
      </p>
    </div>

    <div class="my-4 md:max-w-xl relative">
      <ui-input :type="searching ? 'text' : 'search'" :disabled="loading || searching" v-model="searchQuery" placeholder="Search" class="md:h-12"  @search.prevent="searchQuery?.length > 2 ? searchData() : fetchData()"></ui-input>
